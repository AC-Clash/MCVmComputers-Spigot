...
public class PreventionListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        if (VMComputers.getPlugin().getDB().tableContainsValue(blockS)) {
            e.setCancelled(true);
        ...
        if (VMComputers.getPlugin().getDB().tableContainsValue(componentS)) {
            new BukkitRunnable() {
                @Override
                public void run() {
            ...
            }.runTaskLater(VMComputers.getPlugin(), 1);
