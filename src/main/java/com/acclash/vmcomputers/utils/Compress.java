    ...
    public static void farts() throws IOException {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(VMComputers.getPlugin(), () -> {
            try {
                ...
                VMComputers.getPlugin().getLogger().severe("No Internet: Unable to download Windows 95 image file");
            }
        });
        progress = Bukkit.getScheduler().runTaskTimer(VMComputers.getPlugin(), () -> {
            ...
            if (percent == 1) {
                progress.cancel();
                VMComputers.getPlugin().getLogger().info(ChatColor.GREEN + "Downloaded the Windows 95 image archive. Attempting to decompress it");
                try {
                    decompress7ZipFile(compressedFile, image);
                } catch (IOException e) {
                    VMComputers.getPlugin().getLogger().severe("Unable to download Windows 95 image file. Please report this to the developer!");
                }
            }
        }, 0, 1);
    }
