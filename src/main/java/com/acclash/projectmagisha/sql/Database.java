...
public abstract class Database {
    ProjectMagisha plugin;
    Connection connection;
    // The name of the table we created back in SQLite class.
    public String table = "computers";
    public int tokens = 0;
    public Database(ProjectMagisha instance){
        plugin = instance;
    }
    ...
    public void initialize(){
        ...
        //try{
            //PreparedStatement ps = connection.prepareStatement("SELECT * FROM " + table + " WHERE computer_id = ?");
            //ResultSet rs = ps.executeQuery();
            //close(ps,rs);

        //} catch (SQLException ex) {
          //  ProjectMagisha.getPlugin().getLogger().log(Level.SEVERE, "Unable to retreive connection", ex);
        //}
    }
    ...
    public Integer getTokens(String string) {
        ...
        try {
            ...
            ProjectMagisha.getPlugin().getLogger().log(Level.SEVERE, Errors.sqlConnectionExecute(), ex);
        } finally {
            try {
                if (ps != null)
                    ps.close();
                ...
                ProjectMagisha.getPlugin().getLogger().log(Level.SEVERE, Errors.sqlConnectionClose(), ex);
            }
        }
        return 0;
    }
