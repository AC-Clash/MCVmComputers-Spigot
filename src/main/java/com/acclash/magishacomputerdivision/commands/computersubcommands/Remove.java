        ...
        if (args.length == 1) {
            ...
            try {
                ...
                if (resultSet.next()) {
                    ...
                    Optional<Entity> eChair = blockLoc.getWorld().getNearbyEntities(blockLoc, 1, 1, 1).stream().filter(entity -> entity.getPersistentDataContainer().has(new NamespacedKey(MagishaComputerDivision.getPlugin(), "isEChair"), PersistentDataType.STRING)).findFirst();
    ...
    public List<String> onTabComplete(CommandSender sender, String[] args) {
        if (args.length == 2) {
            ...
            try {
                ...
                rs = MagishaComputerDivision.getPlugin().getDB().executeQuery("SELECT `id` FROM `computers`");
