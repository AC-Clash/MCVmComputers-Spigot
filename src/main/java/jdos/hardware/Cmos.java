...
public class Cmos extends Module_base {
    ...
    static private void cmos_checktimer() {
        ...
        Log.specializedLog(LogType.LOG_PIT, Level.INFO,"RTC Timer at "+ StringHelper.format(1000.0/cmos.timer.delay, 2)+" hz");
    ...
    static private final IoHandler.IO_WriteHandler cmos_writereg = (port, val, iolen) -> {
        ...
        case 0x05:		/* Hours Alarm */
            Log.specializedLog(LogType.LOG_BIOS,Level.INFO,"CMOS:Trying to set alarm");
            cmos.regs[cmos.reg]=(byte)val;
            break;
        case 0x0a:		/* Status reg A */
            cmos.regs[cmos.reg]=(byte)(val & 0x7f);
            if ((val & 0x70)!=0x20) Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"CMOS Illegal 22 stage divider value");
        ...
        case 0x0b:		/* Status reg B */
            ...
            if ((val&0x10)!=0) Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"CMOS:Updated ended interrupt not supported yet");
            cmos_checktimer();
            break;
        ...
        default:
            cmos.regs[cmos.reg]=(byte)(val & 0x7f);
            Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"CMOS:WRite to unhandled register "+Integer.toString(cmos.reg,16));
        }
    };
    ...
    static private final IoHandler.IO_ReadHandler cmos_readreg = (port, iolen) -> {
        if (cmos.reg>0x3f) {
            Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"CMOS:Read from illegal register "+Integer.toString(cmos.reg,16));
            return 0xff;
        }
    ...
    //		Log.specializedLog(LogType.LOG_BIOS,Level.INFO,"CMOS:Read from reg %X : %04X",cmos.reg,cmos.regs[cmos.reg]);
            return cmos.regs[cmos.reg] & 0xFF;
        default:
            Log.specializedLog(LogType.LOG_BIOS,Level.INFO,"CMOS:Read from reg "+Integer.toString(cmos.reg,16));
            return cmos.regs[cmos.reg];
        }
    };
