...
public class DMA extends Module_base {
    ...
    public static class DmaController {
        ...
        public /*Bitu*/int ReadControllerReg(/*Bitu*/int reg,/*Bitu*/int len) {
            ...
            case 0x8:		/* Status Register */
                ...
                return ret;
            default:
                if (Log.level<= LogSeverities.LOG_NORMAL.getValue()) Log.specializedLog(LogType.LOG_DMACONTROL, Level.INFO,"Trying to read undefined DMA port "+Integer.toString(reg,16));
                break;
            }
            return 0xffffffff;
        }
    ...
    static private void DMA_BlockRead(/*PhysPt*/int spage,/*PhysPt*/int offset,short[] data, int dataOffset, /*Bitu*/int size) {
        ...
        for ( ; size!=0 ; size--, offset++) {
            if (offset>(dma_wrapping<<dma16)) {
			    Log.getLogger().info("DMA segbound wrapping (read): "+Long.toString(spage, 16)+":"+Integer.toString(offset,16)+" size "+Integer.toString(size, 16)+" [1] wrap "+Long.toString(dma_wrapping,16));
		    }
            offset &= (int) dma_wrap;
            ...
            if (left) {
                ...
                data[dataOffset++] |= (short) (Memory.phys_readb(page*4096 + (offset & 4095)) << 8);
            }
            left=!left;
        }
    }
    ...
    static private void DMA_BlockRead(/*PhysPt*/int spage,/*PhysPt*/int offset,byte[] data, int dataOffset, /*Bitu*/int size) {
        ...
        for ( ; size!=0 ; size--, offset++) {
            if (offset>(dma_wrapping)) {
			    Log.getLogger().info("DMA segbound wrapping (read): "+Long.toString(spage, 16)+":"+Integer.toString(offset,16)+" size "+Integer.toString(size, 16)+" [0] wrap "+Long.toString(dma_wrapping,16));
		    }
            offset &= (int) dma_wrap;
            /*Bitu*/int page = highpart_addr_page+(offset >>> 12);
            /* care for EMS pageframe etc. */
    ...
    static void DMA_BlockWrite(/*PhysPt*/int spage,/*PhysPt*/int offset,byte[] data, int data_offset, /*Bitu*/int size,/*Bit8u*/short dma16) {
        ...
        for (int i=0; size!=0 ; size--, offset++, i++) {
            if (offset>(dma_wrapping<<dma16)) {
			    Log.getLogger().info("DMA segbound wrapping (write): "+Long.toString(spage, 16)+":"+Long.toString(offset,16)+" size "+Integer.toString(size, 16)+" ["+dma16+"] wrap "+Long.toString(dma_wrapping,16));
		    }
            offset &= (int) dma_wrap;
            /*Bitu*/int page = highpart_addr_page+(offset >>> 12);
            /* care for EMS pageframe etc. */
