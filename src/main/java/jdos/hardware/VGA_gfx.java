...
static boolean index9warned=false;
    ...
    //		LOG_DEBUG("Bit mask %2X",val);
        ...
        default:
            ...
            if (VGA.vga.gfx.index == 9 && !index9warned) {
                Log.specializedLog(LogType.LOG_VGAMISC, Level.INFO,"VGA:3CF:Write "+Integer.toString(val, 16)+" to illegal index 9");
                index9warned=true;
                break;
            }
            Log.specializedLog(LogType.LOG_VGAMISC,Level.INFO,"VGA:3CF:Write "+Integer.toString(val, 16)+" to illegal index "+Integer.toString(VGA.vga.gfx.index,16));
            break;
        }
    };
    ...
    private static final IoHandler.IO_ReadHandler read_p3cf = (port, iolen) -> {
        ...
        default:
            ...
            Log.specializedLog(LogType.LOG_VGAMISC,Level.INFO,"Reading from illegal index "+Integer.toString(VGA.vga.gfx.index, 16)+" in port "+Integer.toString(port,16));
            break;
        }
