...
public class NE2000 extends Module_base {
    ...
    public static final class bx_ne2k_c implements RxFrame {
        ...
        public void init() {
            if (BX_INFO)
                System.out.println(StringHelper.sprintf("[NE2000] port 0x%x/32 irq %d mac %02x:%02x:%02x:%02x:%02x:%02x", new Object[]{
        ...
        public void reset(int type) {
            if (BX_DEBUG) System.out.println("[NE2000] reset");
        ...
        public /*Bit32u*/int read_cr() {
            ...
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] read CR returns 0x%08x", new Object[]{new Integer(val)}));
            return val;
        }

        public void write_cr(/*Bit32u*/int value) {
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] wrote 0x%02x to CR", new Object[]{new Integer(value)}));

            // Validate remote-DMA
            if ((value & 0x38) == 0x00) {
                if (BX_DEBUG)
                    System.out.println("[NE2000] CR write - invalid rDMA value 0");
            ...
            if (s.CR.rdma_cmd == 3) {
                ...
                if (BX_INFO)
                    System.out.println("[NE2000] Sending buffer #x" + Integer.toString(s.remote_start, 16) + " length " + s.remote_bytes);
            }
            ...
            if ((value & 0x04) != 0 && s.TCR.loop_cntl != 0) {
                ...
                if (s.TCR.loop_cntl != 1) {
                    if (BX_INFO)
                        System.out.println("[NE2000] Loop mode " + s.TCR.loop_cntl + " not supported.");
                } else {
                ...
                if (s.CR.stop != 0 || s.CR.start == 0)
                    if (BX_PANIC)
                        System.out.println("[NE2000] CR write - tx start, dev in reset");

                if (s.tx_bytes == 0)
                    if (BX_PANIC)
                        System.out.println("[NE2000] CR write - tx start, tx bytes == 0");
...
//                #endif
                ...
                if (s.tx_timer_active != 0) {
                    if (BX_PANIC)
                        System.out.println("[NE2000] CR write, tx timer still active");
        ...
        public /*Bit32u*/long chipmem_read(/*Bit32u*/int address, /*unsigned*/int io_len) {
            ...
            if ((io_len == 2) && (address & 0x1) != 0)
                if (BX_PANIC)
                    System.out.println("[NE2000] unaligned chipmem word read");
            ...
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("out-of-bounds chipmem read, %04X", new Object[]{new Integer(address)}));
        ...
        public void chipmem_write(/*Bit32u*/int address, /*Bit32u*/long value, /*unsigned*/int io_len) {
            if ((io_len == 2) && (address & 0x1) != 0)
                if (BX_PANIC)
                    System.out.println("[NE2000] unaligned chipmem word write");
            ...
            switch (offset) {
                case 0x0:  // Data register
                    ...
                    if (io_len > s.remote_bytes) {
                        if (BX_ERROR)
                            System.out.println("[NE2000] dma read underrun iolen=" + io_len + " remote_bytes=" + s.remote_bytes);
                        //return 0;
                    }
                ...
                default:
                    if (BX_INFO)
                        System.out.println(StringHelper.sprintf("[NE2000] asic read invalid address %04x", new Object[]{new Integer(offset)}));
                    break;
            }
            return retval;
        }
        ...
        public void asic_write(/*Bit32u*/int offset, /*Bit32u*/long value, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] asic write addr=0x%02x, value=0x%04x", new Object[]{new Integer(offset), new Long(value)}));
            switch (offset) {
                case 0x0:  // Data register - see asic_read for a description

                    if ((io_len == 2) && (s.DCR.wdsize == 0)) {
                        if (BX_PANIC)
                            System.out.println("[NE2000] dma write length 2 on byte mode operation");
                        break;
                    }

                    if (s.remote_bytes == 0)
                        if (BX_PANIC)
                            System.out.println("[NE2000] dma write, byte count 0");
                ...
                default: // this is invalid, but happens under win95 device detection
                    if (BX_INFO)
                        System.out.println(StringHelper.sprintf("[NE2000] asic write invalid address %04x, ignoring", new Object[]{new Long(offset)}));
                    break;
            }
        }
        ...
        public /*Bit32u*/long page0_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] page 0 read from port %04x, len=%d", new Object[]{new Integer(offset), new Integer(io_len)}));
            if (io_len > 1) {
                if (BX_ERROR) {
                    /* encountered with win98 hardware probe */
                    System.out.println(StringHelper.sprintf("[NE2000] bad length! page 0 read from port %04x, len=%u", new Object[]{new Integer(offset), new Integer(io_len)}));
                }
                return 0;
            }
            ...
            switch (offset) {
                ...
                case 0x6:  // FIFO
                    ...
                    if (BX_ERROR)
                        System.out.println("[NE2000] reading FIFO not supported yet");
                    return (s.fifo);
                case 0x7:  // ISR
                ...
                case 0xa:  // reserved
                    if (BX_INFO)
                        System.out.println("[NE2000] reserved read - page 0, 0xa");
                    return (0xff);
                case 0xb:  // reserved
                    if (BX_INFO)
                        System.out.println("[NE2000] reserved read - page 0, 0xb");
                    return (0xff);
                case 0xc:  // RSR
                ...
                default:
                    if (BX_PANIC)
                        System.out.println(StringHelper.sprintf("[NE2000] page 0 read offset %04x out of range", new Object[]{new Integer(offset)}));
            }
            return (0);
        }
        ...
        public void page0_write(/*Bit32u*/int offset, /*Bit32u*/long val, /*unsigned*/int io_len) {
            int value = (int) val;
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] page 0 write to port %04x, len=%d", new Object[]{new Integer(offset), new Integer(io_len)}));
            ...
            switch (offset) {
                ...
                case 0xc:  // RCR
                    ...
                    if ((value & 0xc0) != 0)
                        if (BX_INFO)
                            System.out.println("[NE2000] RCR write, reserved bits set");
                    ...
                    if ((value & 0x20) != 0)
                        if (BX_INFO)
                            System.out.println("[NE2000] RCR write, monitor bit set!");
                    break;
                ...
                case 0xd:  // TCR
                    ...
                    if ((value & 0xe0) != 0)
                        if (BX_ERROR)
                            System.out.println("[NE2000] TCR write, reserved bits set");

                    // Test loop mode (not supported)
                    if ((value & 0x06) != 0) {
                        s.TCR.loop_cntl = (short) ((value & 0x6) >> 1);
                        if (BX_INFO)
                            System.out.println("[NE2000] TCR write, loop mode " + s.TCR.loop_cntl + " not supported");
                    } else {
                        s.TCR.loop_cntl = 0;
                    }
                    ...
                    if ((value & 0x01) != 0)
                        if (BX_PANIC)
                            System.out.println("[NE2000] TCR write, inhibit-CRC not supported");

                    // Auto-transmit disable very suspicious
                    if ((value & 0x08) != 0)
                        if (BX_PANIC)
                            System.out.println("[NE2000] TCR write, auto transmit disable not supported");
                ...
                case 0xe:  // DCR
                    ...
                    if ((value & 0x08) == 0) {
                        if (BX_ERROR)
                            System.out.println("[NE2000] DCR write, loopback mode selected");
                    }
                    // It is questionable to set longaddr and auto_rx, since they
                    // aren't supported on the ne2000. Print a warning and continue
                    if ((value & 0x04) != 0)
                        if (BX_INFO)
                            System.out.println("[NE2000] DCR write - LAS set ???");
                    if ((value & 0x10) != 0)
                        if (BX_INFO)
                            System.out.println("[NE2000] DCR write - AR set ???");
                ...
                case 0xf:  // IMR
                    ...
                    if ((value & 0x80) != 0)
                        if (BX_PANIC)
                            System.out.println("[NE2000] IMR write, reserved bit set");
                    ...
                    if (s.ISR.pkt_tx != 0 && s.IMR.tx_inte != 0) {
                        System.out.println("[NE2000] tx irq retrigger");
                        Pic.PIC_ActivateIRQ(s.base_irq);
                    }
                    break;
                default:
                    if (BX_PANIC)
                        System.out.println("[NE2000] page 0 write, bad offset " + Integer.toString(offset, 16));
            }
        }
        ...
        public /*Bit32u*/long page1_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] page 1 read from port %04x, len=%d", new Object[]{new Integer(offset), new Integer(io_len)}));
            if (io_len > 1) {
                if (BX_ERROR) {
                    /* encountered with win98 hardware probe */
                    System.out.println(StringHelper.sprintf("[NE2000] bad length! page 1 read from port %04x, len=%u", new Object[]{new Integer(offset), new Integer(io_len)}));
                }
                return 0;
            }
            ...
            switch (offset) {
                ...
                case 0x7:  // CURR
                    if (BX_DEBUG)
                        System.out.println("[NE2000] returning current page: " + s.curr_page);
                    return (s.curr_page);
                ...
                default:
                    if (BX_PANIC)
                        System.out.println(StringHelper.sprintf("[NE2000] page 1 read offset %04x out of range", new Object[]{new Integer(offset)}));
            }
            return 0;
        }
        ...
        public void page1_write(/*Bit32u*/int offset, /*Bit32u*/long value, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] page 1 write to port %04x, len=%d", new Object[]{new Integer(offset), new Integer(io_len)}));
            ...
            switch (offset) {
                ...
                default:
                    if (BX_PANIC)
                        System.out.println("[NE2000] page 1 write, bad offset " + Integer.toString(offset, 16));
            }
        }
        ...
        public /*Bit32u*/long page2_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] page 2 read from port %04x, len=%d", new Object[]{new Integer(offset), new Integer(io_len)}));
            if (io_len > 1) {
                if (BX_ERROR) {
                    /* encountered with win98 hardware probe */
                    System.out.println(StringHelper.sprintf("[NE2000] bad length! page 2 read from port %04x, len=%u", new Object[]{new Integer(offset), new Integer(io_len)}));
                }
                return 0;
            }
            ...
            switch (offset) {
                ...
                case 0xb:
                    if (BX_ERROR)
                        System.out.println("reserved read - page 2, 0x" + Integer.toString(offset, 16));
                    return (0xff);
                case 0xc:  // RCR
                ...
                default:
                    if (BX_PANIC)
                        System.out.println(StringHelper.sprintf("[NE2000] page 2 read offset %04x out of range", new Object[]{new Integer(offset)}));
            }
            return 0;
        }
        ...
        public void page2_write(/*Bit32u*/int offset, /*Bit32u*/long value, /*unsigned*/int io_len) {
            ...
            if (offset != 0)
                if (BX_ERROR)
                    System.out.println("[NE2000] page 2 write ?");
            ...
            switch (offset) {
                ...
                case 0x4:
                    if (BX_PANIC)
                        System.out.println("page 2 write to reserved offset 4");
                    break;
                ...
                case 0xf:
                    if (BX_PANIC)
                        System.out.println("[NE2000] page 2 write to reserved offset " + Integer.toString(offset, 16));
                    break;

                default:
                    if (BX_PANIC)
                        System.out.println("[NE2000] page 2 write, illegal offset " + Integer.toString(offset));
                    break;
            }
        }
        ...
        public /*Bit32u*/long page3_read(/*Bit32u*/int offset, /*unsigned*/int io_len) {
            if (BX_PANIC)
                System.out.println("[NE2000] page 3 read attempted");
            return 0;
        }

        public void page3_write(/*Bit32u*/int address, /*Bit32u*/long value, /*unsigned*/int io_len) {
            if (BX_PANIC)
                System.out.println("[NE2000] page 3 write attempted");
        }
...
//        }
        ...
        public void tx_timer() {
            if (BX_DEBUG)
                System.out.println("[NE2000] tx_timer");
            s.TSR.tx_ok = 1;
            // Generate an interrupt if not masked and not one in progress
            if (s.IMR.tx_inte != 0 && s.ISR.pkt_tx == 0) {
                //LOG_MSG("tx complete interrupt");
...
//        }
        ...
        public /*Bit32u*/long read(/*Bit32u*/long address, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                System.out.println(StringHelper.sprintf("[NE2000] read addr %x, len %d", new Object[]{new Long(address), new Integer(io_len)}));
            /*Bit32u*/
            long retval = 0;
            ...
            if (offset >= 0x10) {
                ...
                switch (s.CR.pgsel) {
                    ...
                    default:
                        if (BX_PANIC)
                            System.out.println("[NE2000] unknown value of pgsel in read - " + s.CR.pgsel);
                }
            }
            return retval;
        }
        ...
        public void write(/*Bit32u*/long address, /*Bit32u*/long value, /*unsigned*/int io_len) {
            if (BX_DEBUG)
                System.out.println("[NE2000] write with length " + io_len);
            int offset = (int) (address - s.base_address);
            ...
            if (offset >= 0x10) {
                ...
                switch (s.CR.pgsel) {
                    ...
                    default:
                        if (BX_PANIC)
                            System.out.println("[NE2000] unknown value of pgsel in write - " + s.CR.pgsel);
                }
            }
        }
        ...
        public boolean rx_frame(Ptr buf, /*unsigned*/int io_len) {
            ...
            if (io_len != 60) {
                if (BX_DEBUG)
                    System.out.println("[NE2000] rx_frame with length " + io_len);
            }
            ...
            if (BX_NE2K_NEVER_FULL_RING != 0) {
                if (avail <= pages) {
                    if (BX_DEBUG)
                        System.out.println("[NE2000] no space");
                    return true;
                }
            } else {
                if (avail < pages) {
            ...
            if ((io_len < 60) && s.RCR.runts_ok == 0) {
                if (BX_DEBUG)
                    System.out.println("[NE2000] rejected small packet, length " + io_len);
                return true;
            }
            ...
            if (s.RCR.promisc == 0) {
                ...
                if (BX_DEBUG)
                    System.out.println("[NE2000] rx_frame promiscuous receive");
            }
...
//                     pktbuf[6], pktbuf[7], pktbuf[8], pktbuf[9], pktbuf[10], pktbuf[11]);
    ...
    public NE2000(Section configuration) {
        ...
        if (ethernet == null) {
            System.out.println("Network card disabled. mode="+mode+" not found.");
            load_success = false;
            return;
        }
