...
public class PCI extends Module_base {
    ...
    static private final IoHandler.IO_WriteHandler write_pci_addr = (port, val, iolen) -> {
        Log.specializedLog(LogType.LOG_PCI, Level.INFO,"Write PCI address :="+Integer.toString(val, 16));
        pci_caddress=val;
    };
    ...
    static private final IoHandler.IO_WriteHandler write_pci = (port, val, iolen) -> {
        Log.specializedLog(LogType.LOG_PCI, Level.INFO,"Write PCI data :="+Integer.toString(val, 16)+" (len "+iolen+")");
        ...
        if ((pci_caddress & 0x80ff0000) == 0x80000000) {
            ...
            Log.specializedLog(LogType.LOG_PCI, Level.INFO,"  Write to device "+Integer.toString(devnum, 16)+" register "+Integer.toString(regnum, 16)+" (function "+Integer.toString(fctnum, 16)+") (:="+Integer.toString(val, 16)+")");
            ...
            if (dev==null) return;
            Log.getLogger().info("PCI 0x"+Integer.toHexString(dev.PCIId())+" write "+Integer.toHexString(regnum)+":"+Integer.toHexString(val));
    ...
    static private final IoHandler.IO_ReadHandler read_pci_addr = (port, iolen) -> {
        Log.specializedLog(LogType.LOG_PCI, Level.INFO,"Read PCI address . "+Integer.toString(pci_caddress, 16));
        return pci_caddress;
    };
    ...
    static private final IoHandler.IO_ReadHandler read_pci = (port, iolen) -> {
        Log.specializedLog(LogType.LOG_PCI, Level.INFO,"Read PCI data . "+Integer.toString(pci_caddress, 16));
        ...
        if ((pci_caddress & 0x80ff0000) == 0x80000000) {
            ...
            Log.specializedLog(LogType.LOG_PCI, Level.INFO,"  Read from device "+Integer.toString(devnum, 16)+" register "+Integer.toString(regnum, 16)+" (function "+Integer.toString(fctnum, 16)+"); addr "+Integer.toString(pci_caddress, 16));
                    ...
                    case 1:
                        {
                            ...
                            Log.getLogger().info("PCI 0x"+Integer.toHexString(dev.PCIId())+" read1 "+Integer.toHexString(regnum)+":"+Integer.toHexString(val8));
                            return val8;
                        }
                    case 2:
                        {
                            ...
                            Log.getLogger().info("PCI 0x"+Integer.toHexString(dev.PCIId())+" read2 "+Integer.toHexString(regnum)+":"+Integer.toHexString(val16));
                            return val16;
                        }
                    case 4:
                        {
                            ...
                            Log.getLogger().info("PCI 0x"+Integer.toHexString(dev.PCIId())+" read4 "+Integer.toHexString(regnum)+":"+Integer.toHexString(val32));
                            return val32;
                        }
                    default:
                        break;
    ...
    static final private Callback.Handler PCI_PM_Handler = new Callback.Handler() {
        public int call() {
            Log.getLogger().info("PCI PMode handler, function " + Integer.toString(CPU_Regs.reg_eax.word(), 16));
    ...
    public void RegisterPCIDevice(PCI_Device device) {
        RegisterPCIDevice(device, -1);
    }
	...
	public void RegisterPCIDevice(PCI_Device device, int slot) {
		...
		if (pci_devices[slot]!=null) {
			...
			if (subfunction<0) Log.exit("Too many PCI subdevices!", Level.ERROR);
		}
