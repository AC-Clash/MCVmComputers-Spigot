    ...
    */
    ...
    static private final IoHandler.IO_WriteHandler write_p3c6 = (port, val, iolen) -> {
        if ( VGA.vga.dac.pel_mask != val ) {
            Log.specializedLog(LogType.LOG_VGAMISC, Level.INFO,"VGA:DCA:Pel Mask set to "+Integer.toString(val,16));
...
//		VGA.vga.dac.read_index = VGA.vga.dac.write_index - 1;//disabled as it breaks Wari
            ...
            break;
        default:
            Log.specializedLog(LogType.LOG_VGAGFX,Level.INFO,"VGA:DAC:Illegal Pel Index");			//If this can actually happen that will be the day
            break;
        }
    };
...
//		VGA.vga.dac.write_index=VGA.vga.dac.read_index+1;//disabled as it breaks wari
            break;
        default:
            Log.specializedLog(LogType.LOG_VGAMISC, Level.INFO,"VGA:DAC:Illegal Pel Index");			//If this can actually happen that will be the day
            ret=0;
            break;
        }
        return ret;
