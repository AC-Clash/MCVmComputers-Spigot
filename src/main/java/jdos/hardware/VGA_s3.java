...
//TODO Base address
        ...
        case 0x35:	/* CR35 CRT Register Lock */
            ...
            if (((VGA.vga.svga.bank_read & 0xf) ^ (val & 0xf))!=0) {
                ...
                VGA.vga.svga.bank_read|= (short) (val & 0xf);
        ...
        case 0x4c:  /* HGC start address high byte*/
            ...
            if ((VGA.vga.s3.hgc.startaddr <<10)+((64*64*2)/8) > VGA.vga.vmemsize) {
                ...
                Log.specializedLog(LogType.LOG_VGAMISC, Level.INFO,"VGA:S3:CRTC: HGC pattern address beyond video memory" );
            }
            break;
        ...
        case 0x51:	/* Extended System Control 2 */
            ...
            if (((VGA.vga.svga.bank_read&0x30) ^ ((val&0xc)<<2))!=0) {
                ...
                VGA.vga.svga.bank_read|= (short) ((val&0xc)<<2);
                VGA.vga.svga.bank_write = VGA.vga.svga.bank_read;
                VGA_memory.VGA_SetupHandlers();
            }
        ...
        case 0x6b:	// BIOS scratchpad: LFB address
            ...
            break;
        default:
            Log.specializedLog(LogType.LOG_VGAMISC,Level.INFO,"VGA:S3:CRTC:Write to illegal index "+Integer.toString(reg,16));
            break;
        }
    };
...
//TODO mix in bits from baseaddress;
    ...
    private static final VGA.tWritePort SVGA_S3_WriteSEQ = (reg, val, iolen) -> {
        ...
        case 0x15:
            ...
            break;
        default:
            Log.specializedLog(LogType.LOG_VGAMISC,Level.INFO,"VGA:S3:SEQ:Write to illegal index "+Integer.toString(reg,16));
            break;
        }
    };
    ...
    private static final VGA.tReadPort SVGA_S3_ReadSEQ = (reg, iolen) -> {
        ...
        default:
            Log.specializedLog(LogType.LOG_VGAMISC,Level.INFO,"VGA:S3:SEQ:Read from illegal index "+Integer.toString(reg,16));
            return 0;
        }
    };
