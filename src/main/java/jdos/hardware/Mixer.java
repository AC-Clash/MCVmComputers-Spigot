...
//        }
        ...
        public void AddStretched(/*Bitu*/int len, /*Bit16s*/short[] data) {
            if (done>=needed) {
                Log.getLogger().warn("Can't add, buffer full");
                return;
            }
    ...
    * and removes itself when destroyed. */
    public static class MixerObject{
        ...
        public MixerChannel Install(MIXER_Handler handler,/*Bitu*/int freq,String name) {
            if(!installed) {
                if(name.length() > 31) Log.exit("Too long mixer channel name", Level.ERROR);
                m_name=name;
                installed = true;
                return MIXER_AddChannel(handler,freq,name);
            } else {
                Log.exit("already added mixer channel.", Level.ERROR);
...
//            spec.samples=(Uint16)mixer.blocksize;
            ...
            if (mixer.nosound) {
                Log.getLogger().info("MIXER: No Sound Mode Selected.");
                mixer.tick_add=((long) (mixer.freq) << MIXER_SHIFT)/1000;
...
//                mixer.nosound = true;
//                Log.getLogger().info("MIXER: Can't open audio: %s , running in nosound mode.",SDL_GetError());
//                mixer.tick_add=((mixer.freq) << MIXER_SHIFT)/1000;
//                Timer.TIMER_AddTickHandler(MIXER_Mix_NoSound);
            } else {
//                if((mixer.freq != obtained.freq) || (mixer.blocksize != obtained.samples))
//                    Log.getLogger().info("MIXER: Got different values from SDL: freq %d, blocksize %d",obtained.freq,obtained.samples);
