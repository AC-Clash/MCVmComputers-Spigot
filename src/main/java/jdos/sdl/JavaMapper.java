...
//	new CTextButton(PX(17),0,124,20,"Joystick Layout");
    ...
    public static void CreateStringBind(String in) {
        ...
        if (event == null) {
            Log.getLogger().error("Can't find matching event for " + eventname);
            return ;
        }
        CBind bind;
        while (!line.value.isEmpty()) {
...
//        CreateStringBind("jhat_0_0_3 \"stick_0 hat 0 8\" ");
    ...
    static private void MAPPER_SaveBinds() {
        ...
        try {
            ...
            Log.getLogger().log(Level.ERROR, "Can't open "+fileName+" for saving the mappings ", e);
        }
    }
    ...
    static public boolean MAPPER_LoadBinds(String fileName) {
        ...
        Log.getLogger().info("MAPPER: Loading mapper settings from "+fileName);
        return true;
    }
