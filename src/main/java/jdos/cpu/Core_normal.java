...
public class Core_normal extends Prefix_66_0f {
    ...
    public static final CPU.CPU_Decoder CPU_Core_Normal_Run = () -> {
        //Log.getLogger().info("CPU_Core_Normal_Run");
...
//                    Record.op(c);
//                    if (count>0) {
//                        Log.getLogger().info(String.format("%d %06x:%08x %3s %-8s EAX=%08x ECX=%08x EDX=%08x EBX=%08x ESP=%08x EBP=%08x ESI=%08x EDI=%08x FLAGS=%04x", count, CPU_Regs.reg_csVal.dword, CPU_Regs.reg_eip, Integer.toHexString(c), (desc.length > c ? " " + desc[c] : ""), CPU_Regs.reg_eax.dword, CPU_Regs.reg_ecx.dword, CPU_Regs.reg_edx.dword, CPU_Regs.reg_ebx.dword, CPU_Regs.reg_esp.dword, CPU_Regs.reg_ebp.dword, CPU_Regs.reg_esi.dword, CPU_Regs.reg_edi.dword, CPU_Regs.flags));
//                        count++;
//                        if (count % 5000 == 0) {
//                            int ii = 0;
//                        }
                    ...
                    if (result != HANDLED) {
                        if (result == CONTINUE) {
                            ...
                            if (Log.level<=LogSeverities.LOG_NORMAL.getValue())
                                Log.specializedLog(LogType.LOG_CPU, Level.INFO,"Illegal/Unhandled opcode "+Integer.toHexString(c));
                            CPU.CPU_Exception(6,0);
                            break;
                        }
                    }
