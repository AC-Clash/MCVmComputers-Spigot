...
public class Callback {
    ...
    public static /*Bitu*/int CALLBACK_Allocate() {
        ...
        Log.exit("CALLBACK:Can't allocate handler.", Level.ERROR);
        return 0;
    }
    ...
    private static final Handler illegal_handler = new Handler() {
        public /*Bitu*/int call() {
            Log.exit("Illegal CallBack Called", Level.ERROR);
            return 1;
        }
    ...
    static private final Handler default_handler = new Handler() {
        public /*Bitu*/int call() {
            if (Log.level<=LogSeverities.LOG_ERROR.getValue()) Log.specializedLog(LogType.LOG_CPU, Level.ERROR,"Illegal Unhandled Interrupt Called "+Integer.toString(CPU.lastint,16));
            return CBRET_NONE;
        }
        public String getName() {
    ...
    public static /*Bitu*/int CALLBACK_SetupExtra(/*Bitu*/int callback, /*Bitu*/int type, /*PhysPt*/int lphysAddress, boolean use_cb/*=true*/) {
        ...
        switch (type) {
            ...
            case CB_IRQ12:	// ps2 mouse int74
                if (!use_cb) Log.exit("int74 callback must implement a callback handler!", Level.ERROR);
    ...
    /*	case CB_IPXESR:		// IPX ESR
            if (!use_cb) Log.exit("ipx esr must implement a callback handler!", Level.ERROR);
        ...
        case CB_IPXESR_RET:		// IPX ESR return
            if (use_cb) Log.exit("ipx esr return must not implement a callback handler!", Level.ERROR);
            ...
            case CB_INT13:
                ...
                return (use_cb?9:5);
            default:
                Log.exit("CALLBACK:Setup:Illegal type "+type, Level.ERROR);
        }
        return 0;
    }
    ...
    public void Uninstall() {
        ...
        if(m_type == SETUP) {
            if(vectorhandler.installed){
                ...
                if (Log.level<=LogSeverities.LOG_WARN.getValue()) Log.specializedLog(LogType.LOG_MISC,Level.WARN,"Interrupt vector changed on "+Integer.toString(vectorhandler.interrupt, 16)+" "+CALLBACK_GetDescription(m_callback));
            }
            CALLBACK_RemoveSetup(m_callback);
        } else if(m_type == SETUPAT){
            Log.exit("Callback:SETUP at not handled yet.", Level.ERROR);
        } else if(m_type == NONE){
            //Do nothing. Merely DeAllocate the callback
        } else Log.exit("what kind of callback is this!", Level.ERROR);
    ...
    public void Install(Handler handler,/*Bitu*/int type,String description){
        ...
        } else Log.exit("Callback handler object already installed", Level.ERROR);
    }
    ...
    public void Set_RealVec(/*Bit8u*/int vec){
        ...
        } else Log.exit("double usage of vector handler", Level.ERROR);
    }
