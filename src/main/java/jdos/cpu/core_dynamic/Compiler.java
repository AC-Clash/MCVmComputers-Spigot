...
public class Compiler extends Helper {
    ...
    private static final Thread[] compilerThread;
        ...
        for (int i = 0; i < compilerThread.length; i++) {
            compilerThread[i] = new Thread(() -> {
                try {
                    ...
                    Log.getLogger().log(Level.ERROR, "Could not decode blocks: ", e);
                }
                Log.getLogger().warn("Compiler thread has exited");
            });
        ...
        while (op != null) {
            ...
            if (true) {
                ...
                if (!loopClosed && compile_op(op, alwayUseFastVersion?0:shouldSet, method, (runningEipCount>0)?"CPU_Regs.reg_eip+="+runningEipCount+";":"", seg)) {
                    ...
                    if (op.next != null) {
                        Log.exit("Instruction "+Integer.toHexString(op.c)+" jumped but there was another instruction after it: "+Integer.toHexString(op.next.c), Level.ERROR);
                    }
                }
                if (reset) {
                    seg.reset();
        ...
        if (count >= min_block_size) {
            ...
            if (compiled != null) {
                ...
                if ((compiledMethods % 250)==0) {
                    Log.getLogger().info("Compiled "+compiledMethods+" blocks ("+compilerQueue.size()+" in queue, ave ops/block: "+((float)compiledOps/compiledMethods));
                }
                return compiled;
            }
        }
...
//                }
    ...
    static private Op compileMethod(Op op, StringBuilder method, boolean jump) {
        //Log.getLogger().info(method.toString());
        try {
            ...
            if (saveClasses) {
                if (op instanceof DecodeBlock) {
                    ...
                    Log.exit("Tried to save an incomplete code block", Level.ERROR);
                }
            }
            return compiledCode;
        } catch (Exception e) {
            Log.getLogger().info("Tried to use method: " + method.toString());
            Log.getLogger().log(Level.ERROR, "Could not write to location: ", e);
            if (thowException) {
                Log.getLogger().log(Level.WARN, "Failed to compile: ", e);
            }
        }
        return null;
    }
