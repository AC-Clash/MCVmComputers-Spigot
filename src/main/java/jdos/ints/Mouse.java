...
public class Mouse {
    ...
    static public class _mouse {
        ...
        private void write16uPtr(DataOutputStream os, int[] d) throws IOException {
            ...
            Log.exit("Failed to save mouse state", Level.ERROR);
        }
        ...
        public byte[] save() {
            try {
                ...
                Log.getLogger().log(Level.ERROR, "Could not save byte: ", e);
            }
            return new byte[0];
        }
        ...
        public void load(byte[] data) {
            try {
                ...
                Log.getLogger().log(Level.ERROR, "Could not load byte: ", e);
            }
        }
    ...
    static public void Mouse_NewVideoMode() {
        ...
        if(mode == mouse.mode) {Log.specializedLog(LogType.LOG_MOUSE, Level.INFO,"New video is the same as the old"); /*return;*/}
        ...
        case 0x12:
            ...
            break;
        default:
            Log.specializedLog(LogType.LOG_MOUSE,Level.ERROR,"Unhandled videomode "+Integer.toString(mode, 16)+" on reset");
    ...
    static private final Callback.Handler INT33_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
        //	Log.specializedLog(LogType.LOG_MOUSE,Level.INFO,"MOUSE: %04X %X %X %d %d",reg_ax,reg_bx,reg_cx,POS_X,POS_Y);
            ...
            case 0x07:	/* Define horizontal cursor range */
                {	//lemmings set 1-640 and wants that. iron seeds set 0-640 but doesn't like 640
                    ...
                    Log.specializedLog(LogType.LOG_MOUSE,Level.INFO,"Define Hortizontal range min:"+min+" max:"+max);
                }
                break;
            case 0x08:	/* Define vertical cursor range */
                {	// not sure what to take instead of the Int10_modes.CurMode (see case 0x07 as well)
                    ...
                    Log.specializedLog(LogType.LOG_MOUSE,Level.INFO,"Define Vertical range min:"+min+" max:"+max);
                }
                break;
            ...
            case 0x16: /* Save driver state */
                {
                    Log.specializedLog(LogType.LOG_MOUSE,Level.WARN,"Saving driver state...");
            ...
            case 0x17: /* load driver state */
                {
                    Log.specializedLog(LogType.LOG_MOUSE,Level.WARN,"Loading driver state...");
                    /*PhysPt*/int src = CPU_Regs.reg_esPhys.dword+CPU_Regs.reg_edx.word();
            ...
            case 0x1a:	/* Set mouse sensitivity */
                ...
                Log.specializedLog(LogType.LOG_MOUSE,Level.WARN,"Set sensitivity used with "+CPU_Regs.reg_ebx.word()+" "+CPU_Regs.reg_ecx.word()+" ("+CPU_Regs.reg_edx.word()+")");
                break;
            case 0x1b:	/* Get mouse sensitivity */
                ...
                Log.specializedLog(LogType.LOG_MOUSE,Level.WARN,"Get sensitivity "+CPU_Regs.reg_ebx.word()+" "+CPU_Regs.reg_ecx.word());
                break;
            ...
            case 0x31: /* Get Current Minimum/Maximum virtual coordinates */
                ...
                break;
            default:
                Log.specializedLog(LogType.LOG_MOUSE,Level.ERROR,"Mouse Function "+Integer.toString(CPU_Regs.reg_eax.word(), 16)+" not implemented!");
                break;
            }
            return Callback.CBRET_NONE;
        }
        ...
        public /*Bitu*/int call() {
            if (mouse.events>0) {
                ...
                if ((mouse.sub_mask & mouse.event_queue[mouse.events].type)!=0) {
                    ...
                    if(mouse.in_UIR) Log.specializedLog(LogType.LOG_MOUSE,Level.ERROR,"Already in UIR!");
                    mouse.in_UIR = true;
