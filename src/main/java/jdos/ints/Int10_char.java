...
/* Do some range checking */
        ...
        if (!gotofilling) {
            while (start!=end) {
                ...
                case VGA.M_LIN4:
                    ...
                    // fall-through
                default:
                    Log.specializedLog(LogType.LOG_INT10, Level.ERROR,"Unhandled mode "+Int10_modes.CurMode.type+" for scroll");
                }
            }
        }
        /* Fill some lines */
    ...
    public static void INT10_SetActivePage(/*Bit8u*/short page) {
        ...
        if (page>7) Log.specializedLog(LogType.LOG_INT10,Level.ERROR,"INT10_SetActivePage page "+page);
...
//			if (Int10_modes.CurMode.mode>0x3) goto dowrite;	//Only mode 0-3 are text modes on cga
    ...
    static public void INT10_SetCursorPos(/*Bit8u*/short row,/*Bit8u*/short col,/*Bit8u*/short page) {
        ...
        if (page>7) Log.specializedLog(LogType.LOG_INT10,Level.ERROR,"INT10_SetCursorPos page "+page);
        // Bios cursor pos
    ...
    static public int ReadCharAttr(/*Bit16u*/int col,/*Bit16u*/int row,/*Bit8u*/short page) {
        ...
        default:		/* EGA/VGA don't have a split font-table */
            ...
            Log.specializedLog(LogType.LOG_INT10,Level.ERROR,"ReadChar didn't find character");
            return 0;
        }
    }
    ...
    public static void WriteChar(/*Bit16u*/int col,/*Bit16u*/int row,/*Bit8u*/short page,/*Bit8u*/short chr,/*Bit8u*/short attr,boolean useattr) {
        ...
        if(!useattr) { //Set attribute(color) to a sensible value
            ...
            if(!warned_use){
                Log.specializedLog(LogType.LOG_INT10,Level.ERROR,"writechar used without attribute in non-textmode "+ (char) chr +" "+Integer.toString(chr,16));
                warned_use = true;
            }
            switch(Int10_modes.CurMode.type) {
            case VGA.M_CGA4:
