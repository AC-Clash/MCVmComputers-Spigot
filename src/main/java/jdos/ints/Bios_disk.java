...
public class Bios_disk {
    ...
    static public class imageDisk  {
        ...
        public void close() {
            if (diskimg != null) {
                try {
                    ...
                    Log.getLogger().log(Level.ERROR, "Could not close disk image: ", e);
                }
            }
        }
        ...
        public /*Bit8u*/short Read_AbsoluteSector(/*Bit32u*/long sectnum, byte[] data, int off) {
            ...
            try {
                ...
                Log.getLogger().log(Level.ERROR, "Could not seek byte enum: ", e);
            }
            return 0x00;
        }
        ...
        public /*Bit8u*/short Write_AbsoluteSector(/*Bit32u*/long sectnum, byte[] data, int off) {
            ...
            try {
                ...
                ret = 0;
                Log.getLogger().log(Level.ERROR, "Could not write to disk: ", e);
            }
            ...
            if(!isHardDisk) {
                ...
                while (DiskGeometryList[i].ksize!=0x0) {
                    if ((DiskGeometryList[i].ksize==imgSizeK) ||
                        ...
                        if (DiskGeometryList[i].ksize!=imgSizeK)
                            Log.getLogger().warn("ImageLoader: image file with additional data, might not load!");
                        founddisk = true;
                        active = true;
                        floppytype = i;
    ...
    static public void swapInDisks() {
        ...
        while(diskcount<2) {
            if(diskSwap[swapPos] != null) {
                Log.getLogger().info("Loaded disk "+diskcount+" from swaplist position "+swapPos+" - \""+diskSwap[swapPos].diskname+"\"");
    ...
    static private final Mapper.MAPPER_Handler swapInNextDisk = new Mapper.MAPPER_Handler() {
        public void call(boolean pressed) {
            ...
            Log.getLogger().info("Diskcaching reset for normal mounted drives.");
    ...
    static boolean driveInactive(/*Bitu*/int driveNum) {
        if(driveNum>=(2 + MAX_HDD_IMAGES)) {
            Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"Disk "+driveNum+" non-existant");
            last_status = 0x01;
            Callback.CALLBACK_SCF(true);
            return true;
        }
        if(imageDiskList[driveNum] == null) {
            Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"Disk "+driveNum+" not active");
            last_status = 0x01;
            Callback.CALLBACK_SCF(true);
            return true;
        }
            ...
            case 0x2: /* Read sectors */
                ...
                for(i=0;i<CPU_Regs.reg_eax.low();i++) {
                    ...
                    if((last_status != 0x00) || (killRead)) {
                        Log.getLogger().error("Error in disk read");
                        killRead = false;
    ...
    //                if(last_status != 0x00) {
    //                    Log.getLogger().info("Error in disk read");
    //                    Callback.CALLBACK_SCF(true);
    //                    return Callback.CBRET_NONE;
    //                }
    //                for(t=0;t<512;t++) {
    ...
    //            }
            ...
            case 0x08: /* Get drive parameters */
                ...
                if (tmpcyl.value==0) Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"INT13 DrivParm: cylinder count zero!");
                else tmpcyl.value--;		// cylinder count . max cylinder
                if (tmpheads.value==0) Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"INT13 DrivParm: head count zero!");
            ...
            case 0x17: /* Set disk type for format */
                ...
                break;
            default:
                Log.specializedLog(LogType.LOG_BIOS,Level.ERROR,"INT13: Function "+Integer.toString(CPU_Regs.reg_eax.high(), 16)+" called on drive "+Integer.toString(CPU_Regs.reg_edx.low(), 16)+" (dos drive "+drivenum+")");
                CPU_Regs.reg_eax.high(0xff);
                Callback.CALLBACK_SCF(true);
            }
            return Callback.CBRET_NONE;
