...
public class EMS extends Module_base {
    ...
    private static class device_EMM extends DOS_Device {
        ...
        public boolean Write(byte[] data,/*Bit16u*/IntRef size) {
            Log.specializedLog(LogType.LOG_IOCTL, Level.INFO,"EMS:Write to device");
            return false;
        }
    ...
    private static /*Bit8u*/short EMM_AllocateMemory(/*Bit16u*/int pages, /*Bit16u*/IntRef dhandle, boolean can_allocate_zpages) {
        ...
        if (pages!=0) {
            ...
            if (mem==0) Log.exit("EMS:Memory allocation failure", Level.ERROR);
        }
    ...
    private static /*Bit8u*/void EMM_AllocateSystemHandle(/*Bit16u*/int pages) {
        ...
        if (mem==0) Log.exit("EMS:System handle memory allocation failure", Level.ERROR);
        emm_handles[handle].pages = pages;
        emm_handles[handle].mem = mem;
    }
    ...
    static /*Bit8u*/short EMM_ReallocatePages(/*Bit16u*/int handle,/*Bit16u*/int pages) {
        ...
        if (emm_handles[handle].pages != 0) {
            ...
            /*MemHandle*/int mem = Memory.MEM_AllocatePages(pages*4,false);
            if (mem==0) Log.exit("EMS:Memory allocation failure during reallocation", Level.ERROR);
            emm_handles[handle].mem = mem;
        }
        /* Update size */
        emm_handles[handle].pages=pages;
    ...
    //	LOG_MSG("EMS MapPage handle %d phys %d specializedLog %d",handle,phys_page,log_page);
    ...
    //	LOG_MSG("EMS MapSegment handle %d segment %d specializedLog %d",handle,segment,log_page);
    ...
    private static /*Bit8u*/short EMM_PartialPageMapping() {
        ...
        case 0x02:	/* Get Partial Page Map Array Size */
            ...
            break;
        default:
             Log.specializedLog(LogType.LOG_MISC,Level.ERROR,"EMS:Call "+Integer.toString(CPU_Regs.reg_eax.high(), 16)+" Subfunction "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" not supported");
    ...
    private static final Callback.Handler INT67_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            case 0x57:	/* Memory region */
                ...
                if (CPU_Regs.reg_eax.high()!=0)  Log.specializedLog(LogType.LOG_MISC,Level.ERROR,"EMS:Function 57 move failed");
                break;
            ...
            case 0x5A:              /* Allocate standard/raw Pages */
                if (CPU_Regs.reg_eax.low()<=0x01) {
                    ...
                    CPU_Regs.reg_edx.word(dx.value);
                     ...
                     Log.specializedLog(LogType.LOG_MISC,Level.ERROR,"EMS:Call 5A subfct "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" not supported");
                    CPU_Regs.reg_eax.high(EMM_INVALID_SUB);
                }
                break;
            case 0xDE:		/* VCPI Functions */
                if (!vcpi.enabled) {
                     Log.specializedLog(LogType.LOG_MISC,Level.ERROR,"EMS:VCPI Call "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" not supported");
                    ...
                    case 0x0c: {	/* VCPI Switch from V86 to Protected Mode */
                        ...
                        if (CPU.CPU_LLDT(new_ldt)) Log.getLogger().warn("VCPI:Could not load LDT with "+Integer.toString(new_ldt,16));
                        if (CPU.CPU_LTR(new_tr)) Log.getLogger().warn("VCPI:Could not load TR with "+Integer.toString(new_tr, 16));
        ...
        //				MEM_A20_Enable(true);
                        ...
                        break;
                    default:
                         Log.specializedLog(LogType.LOG_MISC,Level.ERROR,"EMS:VCPI Call "+Integer.toString(CPU_Regs.reg_eax.word(), 16)+" not supported");
                        CPU_Regs.reg_eax.high(EMM_FUNC_NOSUP);
                        break;
                    }
                }
                break;
            default:
                 Log.specializedLog(LogType.LOG_MISC,Level.ERROR,"EMS:Call "+Integer.toString(CPU_Regs.reg_eax.high(), 16)+" not supported");
                CPU_Regs.reg_eax.high(EMM_FUNC_NOSUP);
                break;
            }
    ...
    private static final Callback.Handler VCPI_PM_Handler = new Callback.Handler() {
        ...
        //	LOG_MSG("VCPI PMODE handler, function %x",reg_ax);
            ...
            case 0xDE0C: {		/* VCPI Switch from Protected Mode to V86 */
                ...
                if (CPU.CPU_LLDT(0x08)) Log.getLogger().warn("VCPI:Could not load LDT");
                if (CPU.CPU_LTR(0x10)) Log.getLogger().warn("VCPI:Could not load TR");
        ...
        //		MEM_A20_Enable(false);
                ...
                break;
            default:
                Log.specializedLog(LogType.LOG_MISC,Level.WARN,"Unhandled VCPI-function "+Integer.toString(CPU_Regs.reg_eax.low(), 16)+" in protected mode");
                break;
            }
            return Callback.CBRET_NONE;
        }
        ...
        //		LOG_MSG("v86 monitor caught protection violation at %x:%x, opcode=%x",v86_cs,v86_ip,v86_opcode);
                switch (v86_opcode) {
                        ...
                        switch (v86_opcode) {
                            case 0x20: {	// mov reg,CRx
                                ...
                                if ((rm_val<0xc0) || (rm_val>=0xe8))
                                    Log.exit("Invalid opcode 0x0f 0x20 "+Integer.toString(rm_val, 16)+" caused a protection fault!", Level.ERROR);
                            ...
                            case 0x22: {	// mov CRx,reg
                                ...
                                /*Bitu*/int which=(rm_val >> 3) & 7;
                                if ((rm_val<0xc0) || (rm_val>=0xe8))
                                    Log.exit("Invalid opcode 0x0f 0x22 "+Integer.toString(rm_val, 16)+" caused a protection fault!", Level.ERROR);
                                /*Bit32u*/int crx=0;
                                switch (rm_val&7) {
                                ...
                                break;
                            default:
                                Log.exit("Unhandled opcode 0x0f "+Integer.toString(v86_opcode, 16)+" caused a protection fault!", Level.ERROR);
                        }
                        break;
                    case 0xe4:		// IN AL,Ib
                    ...
                    case 0xf4:		// HLT
                        ...
                        break;
                    default:
                        Log.exit("Unhandled opcode "+Integer.toString(v86_opcode, 16)+" caused a protection fault!", Level.ERROR);
                }
                return Callback.CBRET_NONE;
            }
        ...
        public /*Bitu*/int call() {
            if (CPU_Regs.reg_eax.high() == 0x81) {
                ...
                CPU_Regs.reg_eax.word(0x1);
                    ...
                    Log.specializedLog(LogType.LOG_MISC, Level.WARN, "Unhandled interrupt 4B function " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
            }
            return Callback.CBRET_NONE;
        }
    };
    ...
    EMS(Section configuration) {
        ...
        if (Dosbox.machine== MachineType.MCH_PCJR) {
            ems_type=0;
            Log.getLogger().warn("EMS disabled for PCJr machine");
            return;
        }
