...
// :TODO: this entire class was hard to port with accuracy, it will need lots of testingimport jdos.util.Log;

public class DOS_Drive_Cache {
    Comparator SortByName = (o1, o2) -> ((CFileInfo)o1).shortname.compareTo(((CFileInfo)o2).shortname);

    final Comparator SortByDirName = (o1, o2) -> {
    ...
    final Comparator SortByDirNameRev = (o1, o2) -> {
        CFileInfo a = (CFileInfo)o1;
        CFileInfo b = (CFileInfo)o2;
        if (a.isDir!=b.isDir) return a.isDir?1:-1;
    ...
    final Comparator SortByNameRev = (o1, o2) -> {
    CFileInfo a = (CFileInfo)o1;
    CFileInfo b = (CFileInfo)o2;
    return b.shortname.compareTo(a.shortname);
};
    ...
    public boolean FindFirst(String path, IntRef id) {
        ...
        if (local_findcounter == MAX_OPENDIRS) { //Here is the reset from above.
            // no free slot found...
             Log.specializedLog(LogType.LOG_DOSMISC, Level.ERROR,"DIRCACHE: FindFirst/Next: All slots full. Resetting");
        ...
        //	 Log.specializedLog(LogType.LOG_DOSMISC,Level.ERROR,"DIRCACHE: FindFirst : %s (ID:%02X)",path,dirFindFirstID);
    ...
    public boolean FindNext(/*Bit16u*/int id, StringRef result) {
        ...
        if (Log.level<=LogSeverities.LOG_ERROR.getValue()) Log.specializedLog(LogType.LOG_DOSMISC,Level.ERROR,"DIRCACHE: FindFirst/Next failure : ID out of range: "+Integer.toString(id,16));
        return false;
        }
    ...
    public void SetLabel(String vname,boolean cdrom,boolean allowupdate) {
        ...
        * The label will be set at the first call. */
        ...
        label = l.value;
        if (label==null) label="";
        if (Log.level<= LogSeverities.LOG_NORMAL.getValue()) Log.specializedLog(LogType.LOG_DOSMISC,Level.INFO,"DIRCACHE: Set volume label to "+label);
    }
    ...
    static private class CFileInfo {
        ...
        final Vector	longNameList = new Vector();
    }
    ...
    /*Bit16u*/int GetFreeID(CFileInfo dir) {
        ...
        Log.specializedLog(LogType.LOG_FILES, Level.INFO,"DIRCACHE: Too many open directories!");
        dir.id=0;
        return 0;
    }
