...
public class Drive_fat extends Dos_Drive {
    ...
    public Drive_fat(String sysFilename, /*Bit32u*/long bytesector, /*Bit32u*/long cylsector, /*Bit32u*/long headscyl, /*Bit32u*/long cylinders, /*Bit32u*/long startSector) {
        ...
        if (filesize > 2880) {
            ...
            if (mbrData.magic1 != 0x55 || mbrData.magic2 != (byte) 0xaa)
                Log.getLogger().warn("Possibly invalid partition table in disk image.");
            ...
            for (m = 0; m < 4; m++) {
                ...
                if (mbrData.pentry[m].partSize != 0x00) {
                    Log.getLogger().info("Using partition " + m + " on drive; skipping " + mbrData.pentry[m].absSectStart + " sectors");
                    startSector = mbrData.pentry[m].absSectStart;
                    break;
                }
            }

            if (m == 4) Log.getLogger().warn("No good partition found in image.");
        ...
        if ((bootbuffer.magic1 != 0x55) || (bootbuffer.magic2 != (byte) 0xaa)) {
            ...
            Log.getLogger().warn("Loaded image has no valid magic-numbers at the end!");
        }
        ...
        if (CountOfClusters < 4085) {
            /* Volume is FAT12 */
            Log.getLogger().info("Mounted FAT volume is FAT12 with " + CountOfClusters + " clusters");
            fattype = FAT12;
        } else if (CountOfClusters < 65525) {
            Log.getLogger().info("Mounted FAT volume is FAT16 with " + CountOfClusters + " clusters");
            fattype = FAT16;
        } else {
            Log.getLogger().info("Mounted FAT volume is FAT32 with " + CountOfClusters + " clusters");
            fattype = FAT32;
        }
    ...
    public boolean FindFirst(String _dir, Dos_DTA dta, boolean fcb_findfirst/*=false*/) {
        ...
        if ((attr.value & Dos_system.DOS_ATTR_VOLUME) != 0) //check for root dir or fcb_findfirst
            Log.specializedLog(LogType.LOG_DOSMISC, Level.WARN, "findfirst for volumelabel used on fatDrive. Unhandled!!!!!");
    ...
    private boolean FindNextInternal(/*Bit32u*/long dirClustNumber, Dos_DTA dta, DirEntry foundEntry, String longName) {
        ...
        while (true) {
            ...
            if (!Drives.WildFileCmp(find_name, srch_pattern.value)) {
                if (((!longFileName.isEmpty()) && longFileName.toString().equalsIgnoreCase(longName)))
                    break;
                continue;
            }
