    ...
    static private class AudioFile implements TrackFile {
        ...
        private void reopen() {
            try {
                ...
                Log.getLogger().log(Level.ERROR, "Could not reopen bitstream: ", e);
            }
        }
...
//	}
	...
	public boolean SetDevice(String path, int forceCD) {
        if (LoadCueSheet(path)) return false;
        if (LoadIsoFile(path)) return false;
	...
	public boolean PlayAudioSector(long start,long len) {
        ...
        synchronized (player.mutex) {
            ...
            if(track >= 0 && ((Track)tracks.elementAt(track)).attr == 0x40) {
                Log.specializedLog(LogType.LOG_MISC, Level.WARN,"Game tries to play the data track. Not doing this");
...
// failure
                    ...
                    default:
                        success = parts[0].equals("CDTEXTFILE") || parts[0].equals("FLAGS") || parts[0].equals("ISRC")
                                ...
                                || parts[0].equals("SONGWRITER") || parts[0].equals("TITLE") || parts[0].isEmpty();
                        break;
                }
            ...
            Log.getLogger().log(Level.ERROR, "Could not load que sheet: ", e);
            return false;
        }
        // add last track
	...
	private int GetCueFrame(String part) {
        ...
        if (parts.length == 3) {
            try {
                ...
                Log.getLogger().log(Level.ERROR, "Could not get cure frame: ", e);
            }
        }
        return -1;
    }
