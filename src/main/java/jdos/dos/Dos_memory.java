...
public class Dos_memory {
    ...
    static public void DOS_FreeProcessMemory(/*Bit16u*/int pspseg) {
        ...
        } else if (umb_start!=0xffff) Log.specializedLog(LogType.LOG_DOSMISC, Level.ERROR,"Corrupt UMB chain: "+Integer.toString(umb_start,16));
    ...
    static public boolean DOS_ResizeMemory(/*Bit16u*/int segment,/*Bit16u*/IntRef blocks) {
        if (segment < Dos.DOS_MEM_START+1) {
             Log.specializedLog(LogType.LOG_DOSMISC,Level.ERROR,"Program resizes "+Integer.toString(segment, 16)+", take care");
        }
    ...
    //TODO Check if allowed to free this segment
        if (segment < Dos.DOS_MEM_START+1) {
             Log.specializedLog(LogType.LOG_DOSMISC,Level.ERROR,"Program tried to free "+Integer.toString(segment, 16)+" ---ERROR");
    ...
    static public boolean DOS_LinkUMBsToMemChain(/*Bit16u*/int linkstate) {
        ...
        /*Bit16u*/int umb_start=Dos.dos_infoblock.GetStartOfUMBChain();
        if (umb_start!=UMB_START_SEG) {
            if (umb_start!=0xffff)  Log.specializedLog(LogType.LOG_DOSMISC,Level.ERROR,"Corrupt UMB chain: "+Integer.toString(umb_start,16));
            return false;
        }
        ...
        switch (linkstate) {
            ...
            case 0x0001:	// link
                ...
                break;
            default:
                Log.getLogger().error("Invalid link state "+Integer.toString(linkstate, 16)+" when reconfiguring MCB chain");
                return false;
        }
    ...
    static private final Callback.Handler DOS_default_handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
              Log.specializedLog(LogType.LOG_CPU,Level.ERROR,"DOS rerouted Interrupt Called "+Integer.toString(CPU.lastint,16));
